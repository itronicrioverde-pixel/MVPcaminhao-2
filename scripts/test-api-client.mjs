...
try{
 const {httpApi,ApiError}=await import(pathToFileURL(await compile('lib/api-client.ts')));
 ...
 globalThis.fetch=async()=>new Response('ok',{status:200,headers:{'content-type':'text/plain'}});out=await httpApi.get('/api/text');assert.equal(out,'ok');
 globalThis.fetch=async()=>json({error:'Confira os dados informados.',code:'INVALID_DATA'},422);let e=await throwing(()=>httpApi.get('/api'));assert.ok(e instanceof ApiError);assert.equal(e.status,422);assert.equal(e.code,'INVALID_DATA');assert.equal(e.message,'Confira os dados informados.');names.push('resposta JSON com erro');
 ...
 globalThis.fetch=async()=>new Response(null,{status:200,headers:{'content-type':'text/plain'}});out=await httpApi.get('/api/empty');assert.equal(out,'');
 globalThis.fetch=async()=>new Response(null,{status:204});out=await httpApi.get('/api/empty');assert.equal(out,'');names.push('resposta vazia não é interpretada como JSON');
